...
<x-admin.v1.buttons.reference-btn btnType="action-icon"
                                  url="{{route('brands.edit',$brand->id)}}">
    <x-slot name="title">
...
<x-admin.v1.buttons.modal-btn class="btn btn-sm action-icon" target="delete-{{$brand->id}}">
    <x-slot name="icon">
        <i class="mdi mdi-delete"></i>
    </x-slot>
    <x-slot name="modal">
        <x-admin.v1.modals.actions.submit-or-cancel classes="" id="delete-{{$brand->id}}" header="{{__('texts.delete_brand_header')}}">
            <x-slot name="body">
                {{__('texts.delete_brand_body')}} ,{{$brand->name}}
            </x-slot>
