...
@section('title',__('pages_title.edit_category')."|".$category->name)
@section('content')
    <x-admin.v1.layout.partials.basic-page-header>
        <x-slot name="breadcrumbs">
            ...
            <x-admin.v1.layout.partials.bread-crumb-item title="{{__('labels.categories_edit')}}-{{$category->name}}"
                                                         url=""
                                                         isActive="1"/>
        </x-slot>
        <x-slot name="title">
                    ...
                    <x-admin.v1.form.form title="{{__('forms.edit_category_title')}}-{{$category->name}}"
                        ...
                        <x-slot name="inputs">
                            ...
                            <x-admin.v1.form.select-input multiple="0" size="col-md-6" name="parent_id"
                                ...
                                <x-slot name="options">
                                    ...
                                    @foreach($categories as $id => $name)
                                        <option @if($category->parent_id  == $id) selected
                            ...
                            @foreach(config('translatable.locales') as $local)
                                ...
                                <x-admin.v1.form.text-input errorName="" prepend="" value="{{$category->translate($local)->title}}"
                                                            size="col-md-12" name="{{$local}}[title]"
                                                            title="{{__('labels.title',['local'=>$local])}}"
                                                            placeholder="{{__('placeholders.title',['local'=>$local])}}"/>
                                <x-admin.v1.form.text-area-input prepend="" value="{{$category->translate($local)->description}}"
