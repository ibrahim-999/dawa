...
<x-admin.v1.buttons.modal-btn class="btn btn-sm btn-success" target="GrantAccess-{{$chain->id}}">
    <x-slot name="icon">
        {{__('labels.add')}}
    ...
    <x-slot name="modal">
        <x-admin.v1.modals.form.ajax-form
            classes=""
            id="GrantAccess-{{$chain->id}}"
            header="{{__('texts.grant_chain_access_header')}}"
            fileable="false"
            url="{{route('chains.acl.grant',$chain->id)}}"
