...
<x-admin.v1.buttons.reference-btn btnType="action-icon"
                                  url="{{route('campaigns.edit',$campaign->id)}}">
    <x-slot name="title">
...
<x-admin.v1.buttons.modal-btn class="btn btn-sm action-icon" target="delete-{{$campaign->id}}">
    <x-slot name="icon">
        ...
        <x-admin.v1.modals.actions.submit-or-cancel classes="" id="delete-{{$campaign->id}}" header="{{__('texts.delete_driver_header')}}">
            ...
            <x-slot name="actionBtns">
                <x-admin.v1.buttons.delete-btn btnType="btn-danger waves-effect waves-light"
                                                  url="{{route('campaigns.destroy',$campaign->id)}}">
                    <x-slot name="title">
