...
<x-admin.v1.buttons.reference-btn btnType="action-icon"
                                  url="{{route('offers.edit',$offer->id)}}">
    <x-slot name="title">
...
<x-admin.v1.buttons.modal-btn class="btn btn-sm action-icon" target="delete-{{$offer->id}}">
    <x-slot name="icon">
        <i class="mdi mdi-delete"></i>
    </x-slot>
    <x-slot name="modal">
        <x-admin.v1.modals.actions.submit-or-cancel classes="" id="delete-{{$offer->id}}" header="{{__('texts.delete_coupon_header')}}">
            <x-slot name="body">
                {{__('texts.delete_coupon_body')}} ,{{$offer->title}}
            </x-slot>
            <x-slot name="actionBtns">
                <x-admin.v1.buttons.delete-btn btnType="btn-danger waves-effect waves-light"
                                                  url="{{route('offers.destroy',$offer->id)}}">
                    <x-slot name="title">
