...
<x-admin.v1.buttons.modal-btn class="btn btn-sm action-icon" target="delete-{{$role->id}}">
    <x-slot name="icon">
        <i class="mdi mdi-delete"></i>
    </x-slot>
    <x-slot name="modal">
        <x-admin.v1.modals.actions.submit-or-cancel classes="" id="delete-{{$role->id}}" header="{{__('texts.delete_chain_header')}}">
            <x-slot name="body">
                {{__('texts.delete_chain_body')}} ,{{$role->name}}
            </x-slot>
