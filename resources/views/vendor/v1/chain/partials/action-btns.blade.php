...
<x-admin.v1.buttons.reference-btn btnType="action-icon"
                                  url="{{route('chains.edit',$chain->id)}}">
    <x-slot name="title">
...
<x-admin.v1.buttons.modal-btn class="btn btn-sm action-icon" target="delete-{{$chain->id}}">
    <x-slot name="icon">
        <i class="mdi mdi-delete"></i>
    </x-slot>
    <x-slot name="modal">
        <x-admin.v1.modals.actions.submit-or-cancel classes="" id="delete-{{$chain->id}}" header="{{__('texts.delete_chain_header')}}">
            <x-slot name="body">
                {{__('texts.delete_chain_body')}} ,{{$chain->name}}
            </x-slot>
            <x-slot name="actionBtns">
                <x-admin.v1.buttons.delete-btn btnType="btn-danger waves-effect waves-light"
                                                  url="{{route('chains.destroy',$chain->id)}}">
                    <x-slot name="title">
