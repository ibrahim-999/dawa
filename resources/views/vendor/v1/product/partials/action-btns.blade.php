...
<x-admin.v1.buttons.modal-btn class="btn btn-sm action-icon" target="delete-{{$product->id}}">
    <x-slot name="icon">
        <i class="mdi mdi-delete"></i>
    </x-slot>
    <x-slot name="modal">
        <x-admin.v1.modals.actions.submit-or-cancel classes="" id="delete-{{$product->id}}" header="{{__('texts.delete_product_header')}}">
            <x-slot name="body">
                {{__('texts.delete_product_body')}} ,{{$product->name}}
            </x-slot>
